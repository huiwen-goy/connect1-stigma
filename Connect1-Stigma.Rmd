...
Area Under Curve for Age + HHIE-S = 0.657
...
m.age.hhie <- glm(Purchased_HA ~ Age + HHIE_total, family = binomial("logit"), data = mild)
pred.age.hhie <- predict.glm(m.age.hhie, newdata = subset(mild, select=c('Age', 'HHIE_total')), type = 'response')

pred.risk <- cbind(pred.age, pred.hhie, pred.pta, pred.age.hhie)
    
library(PredictABEL)
plotROC(data = mild, 
  cOutcome = 133, 
  predrisk = pred.risk, 
  labels = c("Age", "HHIE-S", "PTA4 better ear", "Age + HHIE-S"), 
  plottitle = "ROC plots", 
